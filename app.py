...
# Keep track of where CSVs are saved
...
def rank_background_task(task_id, source):
    try:
        ...
        if source == 'sample':
            ...
            input_path = os.path.join(base_dir, 'candidates.jsonl')
            total = 100000
        ...
        top_100 = pipeline.run(input_path, output_path, progress_callback)
        ...
        for item in top_100:
            ...
            # Cache full profile details for the timeline view
        ...
        RESULTS[task_id] = {
            ...
            'stats': {
                ...
                'shortlisted': len(top_100),
                'avg_top10_score': sum(c['score'] for c in candidates_data[:10]) / 10 if len(candidates_data) >= 10 else 0,
                'honeypots_detected': sum(1 for c in candidates_data if c['honeypot_flag'])
            }
        }
...
def get_jd_analysis():
    # Return structured job description data based on config.py / challenges requirements
    jd_data = {
...
def download_csv(task_id):
    ...
    if os.path.exists(csv_path):
        return send_file(csv_path, as_attachment=True, download_name=f"submission_{task_id}.csv", mimetype='text/csv')
...
def validate_csv(task_id):
    ...
    try:
        base_dir = r"c:\Users\LENOVO\Desktop\AI-Hacks\[PUB] India_runs_data_and_ai_challenge\[PUB] India_runs_data_and_ai_challenge\India_runs_data_and_ai_challenge"
        val_path = os.path.join(base_dir, 'validate_submission.py')
